...
int main() {
  ...
  // поместить элемент в дерево push(element) прим. - элемент поместится в
  // нужное место. шаблон предназначен для отправки на проверку, для отладки
  // можете самостоятельно помещать свои элементы напр. push(5); push(20);
  // push(50) и т.д. и работать со своими элементами
