...
void* heap_init(size_t initial_size){
	...
	free_list = (struct mem*) (struct mem*) mmap(0, sizeof(struct mem*), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
...
void* _malloc(size_t query){
	...
	if (heap_start != HEAP_START){
		heap_init(query);
		printf("%p\n", free_list);
		free_list->next_free = heap_start;
		free_list->capacity = 0;
		add_free(heap_start);
	}
	...
	struct mem* free_chunk = find_free(query + sizeof(struct mem) + BLOCK_MIN_SIZE);
	
	if(free_chunk){

		split_chunk = (struct mem*) ( (int64_t) free_chunk + query + sizeof(struct mem));
		split_chunk->capacity = free_chunk->capacity - query - sizeof(struct mem);
		split_chunk->is_free = true;
		split_chunk->next = free_chunk->next;
		free_chunk->next = split_chunk;
		free_chunk->capacity = query;
		free_chunk->is_free = false;
		delete_free(free_chunk);
		add_free(split_chunk);

		//printf("%s\n", "Done");

		return (void*) ( (int64_t) free_chunk + sizeof(struct mem));
...
void add_free(struct mem* chunk){

	struct mem* current_chunk = free_list;
	struct mem* next_chunk = free_list->next_free;

	while(next_chunk){

		if (next_chunk->capacity > chunk->capacity){

			chunk->next_free = next_chunk;
			current_chunk->next_free = chunk;
		...
		next_chunk = current_chunk->next_free;
...
void delete_free(struct mem* chunk){
	...
	while(next_chunk){
		...
		if (next_chunk == chunk){
			...
			current_chunk->next_free = next_chunk->next_free;
			chunk->next_free = NULL;
...
struct mem* find_free(size_t size){
